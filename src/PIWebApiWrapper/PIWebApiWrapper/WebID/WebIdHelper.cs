...
namespace PIWebAPIWrapper.WebID
{
    [Guid("9EAF2B2C-18A3-4638-B7BB-BFDFAACFFCD3")]
    [ComVisible(true)]
    ...
    [Guid("9C1A823A-324F-4735-A5FC-C9D996E07EA7")]
    [ComVisible(true)]
    [ClassInterface(ClassInterfaceType.None)]
    [ComSourceInterfaces(typeof(IWebIdHelper))]
    [ProgId("PIWebAPIWrapper.WebIdHelper")]
    public class WebIdHelper : IWebIdHelper
    {

        public string GenerateWebIdByPath(string path, string type, string ownerType)
        {

            Type objectType = null;
            try
            {
                objectType = Type.GetType("PIWebAPIWrapper.Model." + type);
            }
            catch (Exception)
            {
            ...
            Type owner = null;
            if (string.IsNullOrEmpty(ownerType) == false)
            {
               
                try
                {
                    owner = Type.GetType("PIWebAPIWrapper.Model." + ownerType);
                }
                catch (Exception)
                {
            ...
            ValidateTypeAndOwnerType(objectType, owner);
            string marker = GetMarker(objectType);
            string ownerMarker = GetOwnerMarker(owner);
        ...
        private void ValidateTypeAndOwnerType(Type type, Type ownerType)
        {
            if (type == typeof(PIAttribute))
            {
                ...
                {
                    throw new Exception("PIAttribte owner type must be a PIElement or a PIEventFrame.");
                }
            }
            else if (type == typeof(PIAttributeTemplate))
            {
                ...
                {
                    throw new Exception("PIElementTemplate owner type must be a PIElementTemplate.");
                }
            }
            else if ((type == typeof(PIEnumerationSet)) || (type == typeof(PIEnumerationValue)))
            {
                ...
                {
                    throw new Exception("PIEnumerationSet and  PIEnumerationValue owner type must be a PIDataServer or PIAssetServer.");
                }
            }
            else if (type == typeof(PITimeRule))
            {
                ...
                {
                    throw new Exception("PITimeRule owner type must be a PIAnalysis and PIAnalysisTemplate.");
                }
            }
        }
